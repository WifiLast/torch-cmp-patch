...
__global__ void softmax_kernel_fp32_opt(const float* __restrict__ input, float* __restrict__ output, int rows, int cols) {
    ...
    // LDG.128/STG.128 要求 16 字节对齐。row_input/row_output = base + row_idx*cols
    // 只有当 cols 是 4 的倍数时，每一行相对 base 的偏移量才必然是 16 字节对齐的；
    // 否则 row_idx >= 1 的行会产生 CUDA_ERROR_MISALIGNED_ADDRESS，因此这里按行校验对齐后
    // 再决定是否走向量化路径，未对齐的行整体退化为标量循环（下面的 scalar loop 会覆盖全部 cols）。
    bool row_aligned =
        ((reinterpret_cast<uintptr_t>(row_input) & 0xF) == 0) &&
        ((reinterpret_cast<uintptr_t>(row_output) & 0xF) == 0);
    int vec_cols = row_aligned ? (cols / 4) : 0;
