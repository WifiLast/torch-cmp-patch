...
// -DCONV2D_FP32_CTILE=.. (see ../../setup.py's autotune_kernel_tile()) --
// prefixed per-file so it can't collide with fp32_conv3d.cu's/
// fp32_ConvTranspose2d.cu's own CTILE overrides when extension-wide compile
// flags apply to every .cu file. Block dims (16x16=256) are NOT tied to
// CTILE in this register-blocked design (unlike the GEMM-tiled fp16
// kernels' BLOCK_SIZE = f(BM,BN)) -- only the per-thread accumulator/
// weight-pointer array sizes and the channel-block grid dimension change.
...
// -------------------------------------------------------------------------
// 优化版 Kernel: Ampere 1x2 Spatial + CTILE Channel Tiling
//[N=32, C=128, 128x128]：20ms 151W (measured at the original CTILE=8)
// 1. Thread Coarsening: 每个线程计算 2 个水平相邻像素 (W) x CTILE 个输出通道 (C_out)。
//    这使得权重加载的开销分摊到了 2 个像素上，提升了计算密度。
// 2. Pointer Hoisting: 预先计算 CTILE 个通道的权重指针，避免内层循环的乘法。
// -------------------------------------------------------------------------
...
void launch_conv2d_fp32(
    ...
    // Grid Y: Output Channels (每 block 处理 CTILE 个)
...
// launch_conv2d_fp32 on synthetic data at tools/bench.py's Conv2d shape,
