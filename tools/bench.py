...
def run_suite(dtype_name, dtype):
    ...
    t_torch, t_custom, p_torch, p_custom = benchmark_op(
        ...
        lambda i, w, b: cmpext3.ops.linear(i, w, b),
    ...
    t_torch, t_custom, p_torch, p_custom = benchmark_op(
        ...
        lambda x, y: cmpext3.ops.bmm(x, y),
        (input_bmm, mat2_bmm),
        power_monitor=power_monitor
    )
    ...
    t_torch, t_custom, p_torch, p_custom = benchmark_op(
        ...
        lambda x, w, b: cmpext3.ops.conv2d(x, w, b, stride, padding),
    ...
    # 4. Conv3d [New] -- 3D causal VAE decode / video patch-embed shapes.
    # Dims are representative of a HunyuanVideo-style VAE decoder residual
    # block; adjust C_in3/C_out3/D3/H3/W3 to match the exact shapes your
    # pipeline actually hits for a fully faithful comparison. This is the op
    # that dominates VAE-decode wall time (see cmpext3/__init__.py's module
    # docstring: the native kernel measured ~460ms/call vs cuDNN's
    # ~83ms/call on real decode shapes -- 5.5x slower, which is why conv3d
    # stays behind CMPEXT3_ENABLE_UNVERIFIED_KERNELS=0 by default). This
    # benchmark exists so that number is reproducible instead of a one-off
    # claim.
    N_3d, C_in3, D3, H3, W3 = 1, 512, 8, 32, 32
    C_out3 = 512
    k3 = 3
    stride3, padding3 = 1, 1

    input_conv3d = torch.randn(N_3d, C_in3, D3, H3, W3, device=device, dtype=dtype)
    weight_conv3d = torch.randn(C_out3, C_in3, k3, k3, k3, device=device, dtype=dtype)
    bias_conv3d = torch.randn(C_out3, device=device, dtype=dtype)

    t_torch, t_custom, p_torch, p_custom = benchmark_op(
        "Conv3d",
        lambda x, w, b: F.conv3d(x, w, b, stride=stride3, padding=padding3),
        lambda x, w, b: cmpext3.ops.conv3d(x, w, b, stride3, padding3),
        (input_conv3d, weight_conv3d, bias_conv3d), n_iter=20, power_monitor=power_monitor
    )
    print(f"Op: Conv3d [N={N_3d}, C={C_in3}, {D3}x{H3}x{W3}]")
    print(f"  Torch         : {t_torch:8.3f} ms, Avg Power: {p_torch:6.2f} W")
    print(f"  Custom        : {t_custom:8.3f} ms, Avg Power: {p_custom:6.2f} W")
    print(f"  Speedup       : {t_torch / t_custom:8.1f} x")

    clean_memory(input_conv3d, weight_conv3d, bias_conv3d) # 立即清理


    # 5. ConvTranspose2d [New]
    ...
    t_torch, t_custom, p_torch, p_custom = benchmark_op(
        ...
        lambda x, w, b: cmpext3.ops.conv_transpose2d(x, w, b, stride_t, padding_t,
    ...
    t_torch, t_custom, p_torch, p_custom = benchmark_op(
        ...
        lambda x, s: cmpext3.ops.upsample_scaling(x, s),
    ...
    t_torch, t_custom, p_torch, p_custom = benchmark_op(
        ...
        lambda q,k,v: cmpext3.ops.attention(q, k, v, scale=sm_scale),
        (q, k, v), 
    ...
    t_torch, t_custom, p_torch, p_custom = benchmark_op(
        ...
        lambda x, w: cmpext3.ops.embedding(x, w),
    ...
    t_torch, t_custom, p_torch, p_custom = benchmark_op(
        ...
        lambda x: cmpext3.ops.softmax(x, dim=-1),
        (input_act,), n_iter=50, power_monitor=power_monitor
    )
    ...
    t_torch, t_custom, p_torch, p_custom = benchmark_op(
        ...
        lambda x: cmpext3.ops.swish(x, beta_val),       # 自定义算子
        (input_act,), n_iter=50, power_monitor=power_monitor
    )
    ...
    t_torch, t_custom, p_torch, p_custom = benchmark_op(
        ...
        lambda x: cmpext3.ops.mish(x),                  # 自定义算子
        (input_act,), n_iter=50, power_monitor=power_monitor
    )
    ...
    t_torch, t_custom, p_torch, p_custom = benchmark_op(
        ...
        lambda x: cmpext3.ops.softplus(x, sp_beta, sp_threshold),          # 自定义算子
        (input_act,), n_iter=50, power_monitor=power_monitor
    )
    ...
    t_torch, t_custom, p_torch, p_custom = benchmark_op(
        ...
        lambda x: cmpext3.ops.softsign(x),              # 自定义算子
        (input_act,), n_iter=50, power_monitor=power_monitor
    )
    ...
    t_torch, t_custom, p_torch, p_custom = benchmark_op(
        ...
        lambda x: cmpext3.ops.softshrink(x, lambd_val),     # 自定义算子
        (input_act,), n_iter=50, power_monitor=power_monitor
    )
    ...
    t_torch, t_custom, p_torch, p_custom = benchmark_op(
        ...
        lambda x, w, b: cmpext3.ops.group_norm(x, groups, w, b, 1e-5),
    ...
    t_torch, t_custom, p_torch, p_custom = benchmark_op(
        ...
        lambda x, s, w, b: cmpext3.ops.layer_norm(x, s, w, b, 1e-5),
    ...
    t_torch, t_custom, p_torch, p_custom = benchmark_op(
        ...
        lambda x, s, w: cmpext3.ops.rmsnorm(x, s, w, 1e-6),
