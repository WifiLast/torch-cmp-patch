...
// ---------------------------------------------------------------------------
// SiLU epilogue helpers, ported from fp16_silu.cu (see fp32_groupnorm.cu's
// header comment for why these are duplicated locally instead of shared
// across translation units). Used to fuse GroupNorm's normalize+affine step
// with the SiLU that almost always follows it in a diffusion UNet/VAE block
// (GroupNorm -> SiLU -> Conv), avoiding a separate silu_kernel_fp16 launch
// and its extra global-memory round trip.
// ---------------------------------------------------------------------------
__device__ __forceinline__ half2 gn_silu_h2(half2 x) {
    half2 neg_x = __hneg2(x);
    float2 f2_in = __half22float2(neg_x);
    f2_in.x = __expf(f2_in.x);
    f2_in.y = __expf(f2_in.y);
    half2 e_neg_x = __float22half2_rn(f2_in);
    const half2 one = __float2half2_rn(1.0f);
    half2 den = __hadd2(one, e_neg_x);
    half2 sig = h2rcp(den);
    return __hmul2(x, sig);
}

__device__ __forceinline__ half gn_silu_h1(half x) {
    half neg_x = __hneg(x);
    float f_in = __half2float(neg_x);
    float f_exp = __expf(f_in);
    half e_neg_x = __float2half_rn(f_exp);
    const half one = __float2half_rn(1.0f);
    half den = __hadd(one, e_neg_x);
    half sig = hrcp(den);
    return __hmul(x, sig);
}
...
__global__ void GroupNormKernelFP16(
    ...
    // half2 loads/stores below need `src`/`dst` 4-byte aligned, i.e.
    // group_offset even. With num_groups == C (channels_per_group == 1,
    // as in Wav2Vec2/HuBERT's GroupNorm conv layer) group_offset is
    // g * HxW, which is odd for half the blocks whenever HxW is odd --
    // reinterpret_cast<half2*> on that address is a misaligned CUDA
    // access, not just slower. Fall back to a fully scalar path when
    // that's the case instead of assuming alignment.
    bool can_vec2 = (group_offset % 2) == 0;
    ...
    if (can_vec2) {
        ...
        // Misaligned for half2: plain per-element scalar accumulation.
        for (int idx = threadIdx.x; idx < num_elements_in_group; idx += blockDim.x) {
            float val_f = __half2float(src[idx]);
            local_stats.x = add_rn(local_stats.x, val_f);
            local_stats.y = add_rn(local_stats.y, mul_rn(val_f, val_f));
        }
    }
    ...
    if (can_vec2) {
        ...
        for (; idx < num_h2; idx += blockDim.x) {
            ...
            if (apply_silu) res_h2 = gn_silu_h2(res_h2);
        ...
        if (num_elements_in_group % 2 != 0) {
            ...
            if (threadIdx.x == 0) {
                ...
                // __hfma (scalar)
                half h_res = __hfma(h_val, h_scale, h_bias);
                dst[tail_idx] = apply_silu ? gn_silu_h1(h_res) : h_res;
            }
        }
    } else {
        // Misaligned for half2: plain per-element scalar normalize+store.
        for (int idx = threadIdx.x; idx < num_elements_in_group; idx += blockDim.x) {
            int c_local = idx / HxW;
            int c_global = c_start + c_local;
            ...
            half h_res = __hfma(h_val, h_scale, h_bias);
            dst[idx] = apply_silu ? gn_silu_h1(h_res) : h_res;
        }
    }
}
