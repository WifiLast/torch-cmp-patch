...
__global__ void rmsnorm_fp32_kernel(float* output, const float* input, const float* weight, int cols, float eps) {
    ...
    // LDG.128/STG.128 要求 16 字节对齐。row_in_ptr/row_out_ptr = base + row_idx*cols，
    // 只有当 cols 是 4 的倍数时每一行才必然对齐；否则 row_idx >= 1 的行会产生
    // CUDA_ERROR_MISALIGNED_ADDRESS。按实际指针地址校验对齐（同时覆盖 weight，
    // 它虽不随 row_idx 偏移，但同样通过 float4 读取），未对齐时整体退化为标量循环。
    bool aligned =
        ((reinterpret_cast<uintptr_t>(row_in_ptr) & 0xF) == 0) &&
        ((reinterpret_cast<uintptr_t>(row_out_ptr) & 0xF) == 0) &&
        ((reinterpret_cast<uintptr_t>(weight) & 0xF) == 0);
    int vec_loop_limit = aligned ? (cols / 4) : 0;
