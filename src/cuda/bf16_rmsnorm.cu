...
__global__ void rmsnorm_bf16_kernel(__nv_bfloat16* output, const __nv_bfloat16* input, const __nv_bfloat16* weight, int cols, float eps) {
    ...
    // int4 向量化访问要求 16 字节对齐。row_in/row_out = base + row_idx*cols，只有
    // 当 cols 是 8 的倍数时每一行才必然对齐；否则 row_idx >= 1 的行会产生
    // CUDA_ERROR_MISALIGNED_ADDRESS。按实际指针地址校验对齐（同时覆盖 weight，
    // 它同样通过 int4 读取），未对齐时整体退化为标量循环（vec_end=0）。
    bool aligned =
        ((reinterpret_cast<uintptr_t>(row_in) & 0xF) == 0) &&
        ((reinterpret_cast<uintptr_t>(row_out) & 0xF) == 0) &&
        ((reinterpret_cast<uintptr_t>(weight) & 0xF) == 0);
    int vec_end = aligned ? (cols / 8) * 8 : 0;
    ...
    // 标量尾部处理：覆盖 [vec_end, cols) 范围内的所有剩余元素，按 blockDim.x 分摊
    // (向量化路径关闭时 vec_end=0，即为 [0, cols) 全量，而不只是尾部)。
    for (int j = vec_end + tid; j < cols; j += blockDim.x) {
        float x = bf162float(row_in[j]);
        float prod = __fmul_rn(x, x);
        sum_sq = __fadd_rn(sum_sq, prod);
    }
    ...
    for (int j = vec_end + tid; j < cols; j += blockDim.x) {
        float x = bf162float(row_in[j]);
        float w = bf162float(weight[j]);

        float norm = __fmul_rn(x, local_inv_rms);
        float final_val = __fmul_rn(norm, w);

        row_out[j] = float2bf16(final_val);
    }
}
